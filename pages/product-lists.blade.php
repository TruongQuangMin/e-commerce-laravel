            ...
            <div class="row">
                @foreach ($childCategories as $childCategory)
                    <div class="col-md-auto item-box">
                        <div class="title">
                            <a href="{{ route('product-list', ['slug' => $childCategory->slug]) }}"
                                data-slug="{{ $childCategory->slug }}" data-id="{{ $childCategory->id }}"
                                onclick="getProductListByCategory(event)"
                                style="font-size:20px;font-weight:600px;text-transform:uppercase">{{ $childCategory->title }}</a>
                        </div>
                    </div>
                @endforeach
                    ...
                    success: function(response) {
                        ...
                        if (response.length > 0) {
                            ...
                            $.each(response, function(index, product) {
                                ...
                                productListHTML += '<img src="' + product.photo +
                                ...
                                productListHTML += '<a href="' + '{{ route("product-detail", ["slug" => "PLACEHOLDER"]) }}'.replace('PLACEHOLDER', product.slug) + '">' + product.title + '</a>';
                                productListHTML += '</h3>';

                                productListHTML += '</div>';
                                productListHTML +=
                                    '<span class="item-price text-primary" style="font-size:19px;">' +
                                    product.price + ' $</span>';
